...
begin
  ...
  if exists (
    select 1 from pg_class c join pg_namespace n on n.oid = c.relnamespace
      where n.nspname in ('public','app_private')
        and c.relname in ('security_settings','session_families','app_sessions','login_rate_limits','endpoint_rate_limits','user_credentials','credential_cutover_locks')
  ...
  if has_function_privilege('anon','public.session_create_user(uuid,text,text,text,text,bytea,text)','execute')
     or has_function_privilege('authenticated','public.session_create_user(uuid,text,text,text,text,bytea,text)','execute')
     or not has_function_privilege('service_role','public.session_create_user(uuid,text,text,text,text,bytea,text)','execute') then
...
begin
  perform public.session_create_user(created_id,'Atomic','User','GENERAL',null,lookup,prepared_hash);
  perform public.session_create_user(created_id,'Atomic','User','GENERAL',null,lookup,prepared_hash);
  ...
  begin
    perform public.session_create_user(failed_id,'Failed','User','GENERAL',null,extensions.digest('failed','sha256'),'invalid');
