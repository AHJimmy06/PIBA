...
export const EditSongView: React.FC = () => {
  ...
  if (loading) return (
    <div className="min-h-screen bg-background flex items-center justify-center">
      <div className="text-muted-foreground animate-pulse font-medium">Abriendo libro de cánticos...</div>
    </div>
  );
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="flex items-center gap-6 text-left">
            ...
            <div>
                <h1 className="text-4xl font-extrabold text-foreground tracking-tight">{songId ? 'Editar Canción' : 'Nueva Canción'}</h1>
                <p className="text-muted-foreground text-lg">Define el autor y añade los acordes con precisión.</p>
            </div>
          </div>

          <div className="flex bg-accent p-1.5 rounded-2xl border border-border">
             <Button
                variant={editMode === 'TEXT' ? 'secondary' : 'ghost'}
                onClick={() => setEditMode('TEXT')}
                className={`rounded-xl px-6 ${editMode === 'TEXT' ? 'bg-accent/80 text-foreground shadow-lg' : 'text-muted-foreground'}`}
             >
             ...
             <Button
                ...
                className={`rounded-xl px-6 ${editMode === 'CHORDS' ? 'bg-primary text-primary-foreground shadow-lg shadow-primary/20' : 'text-muted-foreground'}`}
             >
             ...
             <Button
                ...
                className={`rounded-xl px-6 ${editMode === 'PREVIEW' ? 'bg-accent/80 text-foreground shadow-lg' : 'text-muted-foreground'}`}
             >
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          <div className="lg:col-span-1 space-y-8 text-left">
            <Card className="border-border bg-accent backdrop-blur-md rounded-3xl shadow-2xl">
              <CardHeader className="bg-accent/50 border-b border-border">
                <CardTitle className="text-foreground flex items-center gap-3 text-xs uppercase tracking-widest font-black">
                  <Music2 className="h-4 w-4 text-primary" /> Información
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-6 pt-6">
                <div className="space-y-2">
                  <Label className="text-muted-foreground font-bold ml-1">Título</Label>
                  <Input
                    value={title}
                ...
                <div className="space-y-2">
                  <Label className="text-muted-foreground font-bold ml-1">Autor / Artista</Label>
                  <Input
                    value={author}
                ...
                <div className="space-y-2">
                  <Label className="text-muted-foreground font-bold ml-1">Tono Base</Label>
                  <Input
                    value={baseChords}
                    onChange={(e) => setBaseChords(e.target.value)}
                    placeholder="Ej: G"
                    required
                    className="bg-input border-border text-foreground h-12 rounded-xl font-mono focus:ring-primary focus:border-primary"
                  />
                </div>
              </CardContent>
            </Card>
                    ...
                    <CardContent className="pt-4 grid grid-cols-4 gap-2">
                        {COMMON_CHORDS.map(chord => (
                            <Button
                                ...
                                className="bg-accent border-border text-foreground hover:bg-primary hover:text-primary-foreground h-10 font-bold font-mono text-xs"
                            >
                                {chord}
                            </Button>
                        ))}
            ...
            <Button
              ...
              className="w-full h-16 bg-primary hover:bg-primary/90 text-primary-foreground font-black text-xl rounded-2xl shadow-2xl shadow-primary/20 transition-all active:scale-95"
            >
          ...
          <div className="lg:col-span-3 text-left">
            <Card className="border-border bg-accent backdrop-blur-md rounded-3xl shadow-2xl h-full min-h-[600px] flex flex-col">
              ...
              <CardContent className={`flex-1 relative ${editMode === 'PREVIEW' ? 'p-10 md:p-16 overflow-y-auto custom-scrollbar' : 'p-0'}`}>
                {editMode === 'PREVIEW' ? (
                  ...
                  <Textarea
                    ...
                    required
                    className={`w-full h-full min-h-[600px] bg-transparent border-none text-foreground p-8 font-sans text-xl md:text-2xl leading-relaxed custom-scrollbar focus-visible:ring-0 resize-none ${editMode === 'CHORDS' ? 'cursor-crosshair caret-primary' : ''}`}
                  />
                )}
              </CardContent>
            </Card>
